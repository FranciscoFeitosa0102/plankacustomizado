...
const createSocketEventsChannel = () =>
  eventChannel((emit) => {
    ...
    const handleCardChatMessage = ({ message }) => {
      emit(entryActions.receiveCardChatMessage(message));
    };
    ...
    socket.on('card-chat-message', handleCardChatMessage);
    ...
    return () => {
      ...
      socket.off('card-chat-message', handleCardChatMessage);
